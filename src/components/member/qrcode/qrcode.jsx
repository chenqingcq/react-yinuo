    ...
    createQrcode() {
        ...
        axios.get(lib.Api.memberURL + '/member/memberInfo/getByPhone?phone=' + this.state.phone,
            ...
            if (res.data.code == 0) {
                ...
                QRCode.toCanvas(`http://www.yinuoshangcheng.com?phone=${this.state.phone}`, { errorCorrectionLevel: 'H' }, (err, canvas) => {
                    if (err) throw err;
                    ...
                    <div className='logo' ref='qrcode'>
                        ...
                        </div>
                        {
                            !this.state.hasCanvas ?
                                (
                                    <div className='thumb'><img src={require('../../../assets/img/thumb@2x.png')} />在下方输入手机号立即生成二维码</div>
                                ) :
                                (
                                    <div className='thumb'>欢迎加入一诺商城!</div>
                                )
                        }
                    </div>
                </div>
